...
public function createOne(EnrollmentRequest $request)
{
    ...
    try {
        ...
        // Retrieve the enrollment record after attaching
        $enrollment = $course->students()->find($studentId);

        return response()->json(['message' => 'Enrollment created successfully', 'status' => 201, 'enrollment' => $enrollment]);
    } catch (\Exception $ex) {
